 ...
 * ChatLauncher
 * ──────────────────────────────────────────────────────────────────────
 * Floating chat widget — one tap opens the agent's platform directly.
 * No text input. A pre-written opener is silently injected into the URL.
 * Viber / Messenger (no URL pre-fill): opener is copied to clipboard.
 */

/* ─── Pre-written opener injected into every platform link ──────────── */
const OPENER = "Hi! I'm interested in learning more about financial protection from BSQ. Can you help me?"
...
export function MessengerChat() {
  ...
  function handleOpen(platform: ChatPlatform, url: string) {
    // No pre-fill support — copy opener to clipboard so user can paste
    if (!PLATFORM[platform].supportsPreFill) {
      navigator.clipboard.writeText(OPENER).catch(() => {})
    }
  ...
  // Build platform URLs with the pre-written opener silently injected
  const platforms = getAvailablePlatforms(contact, OPENER)
      ...
      {open && (
        <div style={{
          ...
          <style>{`
            ...
            .bsq-platform-btn:hover { opacity: 0.85; transform: translateY(-2px); }
          `}</style>
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ color:'rgba(255,255,255,0.75)', fontSize:'11px', margin:0 }}>Typically replies instantly</p>
            </div>
            <button
          ...
          <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {sent && sentPlatform ? (
              ...
              <div style={{ textAlign:'center', padding:'12px 0' }}>
                ...
                <p style={{ color:'#fff', fontWeight:700, fontSize:'14px', margin:'0 0 6px' }}>
                ...
                <p style={{ color:'rgba(255,255,255,0.55)', fontSize:'12px', margin:0, lineHeight:1.5 }}>
                  {PLATFORM[sentPlatform].supportsPreFill
                    ? 'Your message is ready — just tap Send.'
                    : 'Message copied to clipboard — paste it and tap Send.'}
                </p>
              </div>
              ...
              /* ── Platform picker ── */
              <>
                <p style={{ color:'rgba(255,255,255,0.45)', fontSize:'12px', margin:0, textAlign:'center' }}>
                ...
                {platforms.length > 1 && (
                  <p style={{ color:'rgba(255,255,255,0.25)', fontSize:'10px', margin:0, textAlign:'center' }}>
                    Tap your preferred app above
                  </p>
                )}
              </>
            )}
